...
function Kinobox(n, t) {
  ...
  try {
    ...
    // Полная блокировка рекламы через CSS
    const style = document.createElement('style');
    style.textContent = `
      /* Полная блокировка рекламных блоков */
      .ads-container, .ad-container, #ads, #ad-block, .ad-overlay,
      [class*="ads"], [id*="ads"], [class*="banner"], [id*="banner"],
      [class*="reklama"], [id*="reklama"], [class*="adv"], [id*="adv"],
      iframe[src*="ads"], iframe[src*="casino"], iframe[src*="bet"],
      iframe[src*="banner"], iframe[src*="reklama"], iframe[src*="adv"],
      div[id^="yandex_rtb"], div[id^="adfox"], 
      div[class^="banner"], div[id^="banner"],
      div[class*="advertising"], div[id*="advertising"],
      div[class*="commercial"], div[id*="commercial"] {
        display: none !important;
      }
      
      /* Полная блокировка всплывающих окон */
        ...
        mutation.addedNodes.forEach((node) => {
          if (node.tagName === 'SCRIPT') {
            ...
            if (src.includes('ads') || src.includes('banner') || 
                src.includes('reklama') || src.includes('adv') ||
                src.includes('casino') || src.includes('bet')) {
              node.remove();
            }
          }
        });
  ...
  this.getSearchUrl = function () {
    ...
    Math.random() < 0.1 && n.set("r", window.location.href);
...
Kinobox.prototype.showIframe = function (n) {
  ...
  // Полная блокировка рекламы в iframe
  t.addEventListener('load', function() {
    try {
      ...
      // Блокировка рекламных скриптов в iframe
      ...
      // Очистка рекламных элементов
      const adElements = iframeDoc.querySelectorAll('[class*="ads"],[id*="ads"],[class*="banner"],[id*="banner"]');
...
Kinobox.prototype.init = function () {
  ...
  const n = this.getSearchUrl(),
    t = function (n) {
      ...
      this.state.events.playerLoaded &&
        this.state.events.playerLoaded(t, n.data);
      document.dispatchEvent(
        new CustomEvent("KinoboxPlayerLoaded", {
          detail: { status: t, sources: n.data },
        })
      );
    }.bind(this);
  this.fetch(n, t);
